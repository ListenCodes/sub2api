...
function sourceKeys(): string[] {
  ...
  return Object.entries(sourceFiles)
    .filter(([path, source]) => {
      if (path.includes('/__tests__/') || /\.(spec|test)\.ts$/.test(path)) return false

      // Component-scoped messages are validated by the component's own tests;
      // their keys are intentionally not part of the global locale contract.
      return !/useScope\s*:\s*['"]local['"]/.test(source)
    })
