<template>
  ...
  >
    ...
    <div class="sidebar-header" :class="{ 'sidebar-header-collapsed': sidebarCollapsed }">
      <!-- Custom Logo or Default Logo — clickable to home -->
      <router-link to="/" class="sidebar-logo flex h-9 w-9 items-center justify-center overflow-hidden rounded-xl shadow-glow" title="Back to Home">
        <img v-if="settingsLoaded" :src="siteLogo || '/logo.png'" alt="Logo" class="h-full w-full object-contain" />
      </router-link>
      <div class="sidebar-brand" :class="{ 'sidebar-brand-collapsed': sidebarCollapsed }" :aria-hidden="sidebarCollapsed ? 'true' : 'false'">
        <router-link to="/" class="sidebar-brand-title text-lg font-bold text-gray-900 dark:text-white hover:text-primary-500 dark:hover:text-primary-400 transition-colors" style="text-decoration:none">
          {{ siteName }}
        </router-link>
        <!-- Version Badge -->
...
const adminNavItems = computed((): NavItem[] => {
  const baseItems: NavItem[] = [
    ...
    { path: '/admin/proxies', label: t('nav.proxies'), icon: ServerIcon },
    {
      ...
      expandOnly: true,
      children: [
        { path: '/admin/risk-control/overview', label: t('nav.userRiskControl'), icon: ShieldIcon },
        { path: '/admin/risk-control/cases', label: t('nav.registrationReview'), icon: UsersIcon },
        { path: '/admin/risk-control/events', label: t('nav.riskEvents'), icon: ChartIcon },
        { path: '/admin/risk-control/subjects', label: t('nav.riskSubjects'), icon: UsersIcon },
        { path: '/admin/risk-control/scenarios', label: t('nav.riskScenarios'), icon: FolderIcon },
        { path: '/admin/risk-control/lists', label: t('nav.riskLists'), icon: ShieldIcon },
        { path: '/admin/risk-control/audit', label: t('nav.riskAudit'), icon: ChartIcon },
