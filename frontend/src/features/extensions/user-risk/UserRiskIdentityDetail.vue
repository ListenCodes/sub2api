  ...
  <div>
    ...
    <section v-else-if="activeTab === 'summary' && summary" class="py-5">
      <div class="grid grid-cols-2 gap-3">
        ...
        <div class="border border-gray-200 p-3 dark:border-dark-700"><p class="text-xs text-gray-500">{{ t('admin.userRiskControl.mode') }}</p><p class="mt-1 text-lg font-semibold text-amber-700 dark:text-amber-300">Shadow</p></div>
      </div>
      <div class="mt-4 divide-y divide-gray-200 border-y border-gray-200 dark:divide-dark-700 dark:border-dark-700">
        <div v-for="domain in summary.domains" :key="domain.domain" class="py-3 text-sm">
          ...
          <ul v-if="domain.signals?.length" class="mt-2 space-y-1 text-xs text-gray-500"><li v-for="signal in domain.signals" :key="`${signal.rule_code}-${signal.occurred_at}`"><span class="font-mono text-gray-700 dark:text-gray-300">{{ signal.rule_code }}</span> · {{ t('admin.userRiskControl.identityEvidenceCount', { count: signal.evidence_count }) }} · {{ formatDate(signal.occurred_at) }}</li></ul>
        </div>
      </div>
    ...
    <section v-else-if="activeTab === 'ip'" class="py-4">
      ...
      <div v-if="ipItems.length" class="divide-y divide-gray-200 border-y border-gray-200 dark:divide-dark-700 dark:border-dark-700">
        <article v-for="item in ipItems" :key="item.id" class="py-3">
          <div class="flex items-start justify-between gap-3"><div><p class="font-mono text-sm font-semibold text-gray-900 dark:text-white">{{ item.ip }}</p><p class="mt-1 text-xs text-gray-500">{{ locationLabel(item) }}</p></div><span class="text-xs text-gray-500">{{ t('admin.userRiskControl.drawer.accounts', { count: item.associated_account_count }) }}</span></div>
          <p class="mt-2 text-xs text-gray-500">{{ item.ip_source }} · {{ item.geo_source || '-' }} · ASN {{ item.asn || '-' }} · {{ t('admin.userRiskControl.identityEventsTotal', { count: item.registration_success_count + item.login_success_count + item.api_success_count }) }} · {{ formatDate(item.last_seen_at) }}</p>
        </article>
...
function healthClass(state: IdentityDomainState) { return ['rounded px-2 py-1', state === 'healthy' ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-300' : state === 'disabled' ? 'bg-gray-100 text-gray-600 dark:bg-dark-700 dark:text-gray-300' : 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-300'] }
function deviceKindLabel(kind: DeviceIdentity['identity_kind']) { return t(`admin.userRiskControl.drawer.deviceKind.${kind}`) }
function relationLabel(value: AssociatedRiskUser['relation']) { return t(`admin.userRiskControl.drawer.relation.${value}`) }
