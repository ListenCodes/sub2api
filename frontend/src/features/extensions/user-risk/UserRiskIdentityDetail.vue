<template>
  <div>
    <div v-if="health" class="flex flex-wrap items-center gap-2 border-b border-gray-200 pb-3 text-xs dark:border-dark-700" data-testid="identity-health-strip">
      <span class="font-medium text-gray-700 dark:text-gray-200">{{ unhealthyDomains.length ? '身份检测存在异常' : '身份检测正常' }}</span>
	  <span class="text-gray-500 dark:text-gray-400">{{ health.mode === 'enforce' ? '综合规则仅处置新注册候选账号' : '当前仅记录和人工复核' }}</span>
      <span v-for="domain in unhealthyDomains" :key="domain" :class="healthClass(health.domains[domain])">{{ domainLabel(domain) }} · {{ stateLabel(health.domains[domain]) }}</span>
    </div>
    ...
    <section v-else-if="activeTab === 'summary' && summary" class="py-5">
      <div data-testid="risk-conclusion" class="border-y border-gray-200 py-4 text-sm dark:border-dark-700">
        ...
        <p v-if="primarySignal" class="mt-1 text-gray-600 dark:text-gray-300">风险证据强度：{{ evidenceStrength(primarySignal) }}。{{ currentDecisionSummary }}</p>
        ...
        <p v-else class="mt-1 text-gray-500">当前有效信号为 0，无需执行账号处置。历史弱关联可在“关联账号”中查看，但不参与当前风险判断。</p>
      </div>
      <div v-if="summary.overall_score > 0 || (summary.historical_max_score || 0) > 0" class="mt-4 grid grid-cols-2 gap-3" data-testid="risk-score-summary">
      ...
      <div v-if="meaningfulDomains.length" class="mt-4 divide-y divide-gray-200 border-y border-gray-200 dark:divide-dark-700 dark:border-dark-700" data-testid="risk-domain-summary">
        <div v-for="domain in meaningfulDomains" :key="domain.domain" class="py-3 text-sm">
      ...
      <details v-if="summary.legacy_notice" class="mt-4 text-xs leading-5 text-gray-500 dark:text-gray-400"><summary class="cursor-pointer">历史数据说明</summary><p class="mt-2">{{ summary.legacy_notice }}</p></details>
    </section>
    ...
    <section v-else-if="activeTab === 'ip'" class="py-4">
      ...
      <div v-if="ipItems.length" class="divide-y divide-gray-200 border-y border-gray-200 dark:divide-dark-700 dark:border-dark-700">
        <article v-for="item in ipItems" :key="item.id" class="py-3">
			...
			<div class="mt-2 flex flex-wrap items-center justify-between gap-2 text-xs"><span :class="item.network_label ? 'text-emerald-700 dark:text-emerald-300' : 'text-gray-500'">网络标签：{{ networkLabelName(item.network_label) }}<template v-if="item.network_label_reason"> · {{ item.network_label_reason }}</template></span><button type="button" class="btn btn-secondary btn-sm" :data-testid="`label-network-${item.id}`" @click="openNetworkLabel(item)">管理标签</button></div>
	...
	<BaseDialog :show="Boolean(labelTarget)" title="共享网络标签" width="narrow" :close-on-click-outside="true" :z-index="80" @close="closeNetworkLabel">
		<div v-if="labelTarget" class="space-y-4" data-testid="network-label-dialog">
...
import { userRiskControlV2API, type AssociatedRiskUser, type DeviceIdentity, type IdentityDomain, type IdentityDomainState, type IdentityHealth, type IdentitySummary, type IPIdentity, type NetworkLabelImpact, type SharedNetworkLabel } from '@/api/admin/userRiskControlV2'
...
const labelTarget = ref<IPIdentity | null>(null), labelDraft = ref<SharedNetworkLabel>('unknown'), labelReason = ref(''), labelPreview = ref<NetworkLabelImpact | null>(null), labelSaving = ref(false), labelError = ref(''), labelActionError = ref('')
const labelRevokePreview = ref(false)
const networkLabelOptions: Array<Record<string, unknown>> = [{ value: 'trusted', label: '可信共享环境', kind: 'group', disabled: true }, { value: 'home', label: '家庭网络' }, { value: 'company', label: '公司网络' }, { value: 'school', label: '学校网络' }, { value: 'trusted_egress', label: '可信出口' }, { value: 'mobile_cgnat', label: '移动 CGNAT' }, { value: 'risk', label: '风险或未知', kind: 'group', disabled: true }, { value: 'public_proxy', label: '公共代理' }, { value: 'unknown', label: '未知' }]
let labelPreviewRequest = 0
...
const meaningfulDomains = computed(() => summary.value?.domains.filter((domain) => domain.score > 0 || domain.signal_count > 0 || (domain.historical_max_score || 0) > 0) || [])
const unhealthyDomains = computed(() => health.value ? domainOrder.filter((domain) => health.value?.domains[domain] !== 'healthy') : [])
...
const currentDecisionSummary = computed(() => health.value?.mode === 'enforce' && primarySignal.value?.rule_code === 'v2_registration_composite_accounts' ? '综合注册规则可在达到阈值时仅拒绝新的候选账号，不会自动封禁已有账号。' : '当前有效动作是记录或人工复核，不会自动拒绝或封禁。')
...
async function load(tab: TabID, force = false) { const state = states[tab]; if ((state.loaded && !force) || state.loading) return; const api = userRiskControlV2API as Partial<typeof userRiskControlV2API>; if (tab === 'summary' && (!api.getUserIdentitySummary || !api.getIdentityHealth)) { state.loaded = true; return } const request = ++state.request; state.loading = true; state.error = ''; try { if (tab === 'summary') { const healthData = await userRiskControlV2API.getIdentityHealth(); if (request !== state.request) return; health.value = healthData; if (!healthData.admin_enabled) { state.loaded = true; return } const summaryData = await userRiskControlV2API.getUserIdentitySummary(props.userId, viewSession.value); if (request !== state.request) return; summary.value = summaryData } else if (health.value && !health.value.admin_enabled) { state.loaded = true; return } else if (tab === 'ip') { const data = await userRiskControlV2API.listUserIPIdentities(props.userId, state.page, state.pageSize, appliedIPQuery.value, viewSession.value); if (request !== state.request) return; ipItems.value = data.items; ipTotal.value = data.total } else if (tab === 'device') { const data = await userRiskControlV2API.listUserDeviceIdentities(props.userId, state.page, state.pageSize, viewSession.value); if (request !== state.request) return; deviceItems.value = data.items; deviceTotal.value = data.total } else { const data = await userRiskControlV2API.listAssociatedUsers(props.userId, state.page, state.pageSize, viewSession.value); if (request !== state.request) return; associatedItems.value = data.items; associatedTotal.value = data.total } state.loaded = true } catch (error) { if (request === state.request) state.error = error instanceof Error ? error.message : t('admin.userRiskControl.loadFailed') } finally { if (request === state.request) state.loading = false } }
...
function networkLabelName(value?: SharedNetworkLabel) { return String(networkLabelOptions.find((option) => option.value === value)?.label || '未标记') }
function openNetworkLabel(item: IPIdentity) { labelTarget.value = item; labelDraft.value = item.network_label || 'unknown'; labelReason.value = ''; labelPreview.value = null; labelRevokePreview.value = false; labelError.value = ''; labelActionError.value = '' }
function closeNetworkLabel() { if (!labelSaving.value) labelTarget.value = null }
function resetLabelPreview() { labelPreviewRequest += 1; labelPreview.value = null; labelRevokePreview.value = false }
async function previewNetworkLabel() { if (!labelTarget.value) return; const request = ++labelPreviewRequest; const networkID = labelTarget.value.id; const proposed = labelDraft.value; labelSaving.value = true; labelRevokePreview.value = false; labelActionError.value = ''; try { const impact = await userRiskControlV2API.previewNetworkLabel(networkID, proposed); if (request === labelPreviewRequest && labelTarget.value?.id === networkID && labelDraft.value === proposed) labelPreview.value = impact } catch (error) { if (request === labelPreviewRequest) labelActionError.value = error instanceof Error ? error.message : '影响预览失败' } finally { if (request === labelPreviewRequest) labelSaving.value = false } }
async function previewNetworkLabelRevoke() { if (!labelTarget.value?.network_label) return; const request = ++labelPreviewRequest; const networkID = labelTarget.value.id; labelSaving.value = true; labelActionError.value = ''; try { const impact = await userRiskControlV2API.previewNetworkLabel(networkID, ''); if (request === labelPreviewRequest && labelTarget.value?.id === networkID) { labelPreview.value = impact; labelRevokePreview.value = true } } catch (error) { if (request === labelPreviewRequest) labelActionError.value = error instanceof Error ? error.message : '撤销预览失败' } finally { if (request === labelPreviewRequest) labelSaving.value = false } }
async function applyNetworkLabel() { if (!labelTarget.value || !labelPreview.value || labelRevokePreview.value || labelPreview.value.proposed_label !== labelDraft.value) return; if (!labelReason.value.trim()) { labelError.value = '原因不能为空'; return } labelSaving.value = true; labelActionError.value = ''; try { await userRiskControlV2API.applyNetworkLabel(labelTarget.value.id, labelDraft.value, labelReason.value); const item = ipItems.value.find((candidate) => candidate.id === labelTarget.value?.id); if (item) { item.network_label = labelDraft.value; item.network_label_reason = labelReason.value.trim() } labelTarget.value = null; states.summary.loaded = false } catch (error) { labelActionError.value = error instanceof Error ? error.message : '标签应用失败' } finally { labelSaving.value = false } }
async function revokeNetworkLabel() { if (!labelTarget.value?.network_label || !labelRevokePreview.value || !labelPreview.value) return; if (!labelReason.value.trim()) { labelError.value = '撤销原因不能为空'; return } labelSaving.value = true; labelActionError.value = ''; try { await userRiskControlV2API.revokeNetworkLabel(labelTarget.value.id, labelReason.value); const item = ipItems.value.find((candidate) => candidate.id === labelTarget.value?.id); if (item) { item.network_label = undefined; item.network_label_reason = undefined } labelTarget.value = null } catch (error) { labelActionError.value = error instanceof Error ? error.message : '标签撤销失败' } finally { labelSaving.value = false } }
...
watch(() => props.userId, () => { viewSession.value = typeof crypto !== 'undefined' && 'randomUUID' in crypto ? crypto.randomUUID() : `view-${Date.now()}-${Math.random().toString(36).slice(2)}`; for (const state of Object.values(states)) { state.request += 1; state.loaded = false; state.loading = false; state.error = ''; state.page = 1 } activeTab.value = 'summary'; emit('tab-change', 'summary'); summary.value = null; health.value = null; ipSearchInput.value = ''; appliedIPQuery.value = ''; labelTarget.value = null; labelRevokePreview.value = false; void load('summary') }, { immediate: true })
</script>
