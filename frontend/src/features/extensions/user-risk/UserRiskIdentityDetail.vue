...
watch(() => props.userId, () => { viewSession.value = typeof crypto !== 'undefined' && 'randomUUID' in crypto ? crypto.randomUUID() : `view-${Date.now()}-${Math.random().toString(36).slice(2)}`; for (const state of Object.values(states)) { state.request += 1; state.loaded = false; state.loading = false; state.error = ''; state.page = 1 } activeTab.value = 'summary'; emit('tab-change', 'summary'); summary.value = null; health.value = null; ipSearchInput.value = ''; appliedIPQuery.value = ''; technicalExpanded.value = new Set(); labelTarget.value = null; labelRevokePreview.value = false; void load('summary') }, { immediate: true })
</script>
