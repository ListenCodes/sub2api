<template>
  <div>
    <div v-if="health" class="flex flex-wrap items-center gap-2 border-b border-gray-200 pb-3 text-xs dark:border-dark-700">
      ...
      <span :class="health.mode === 'enforce' ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-300' : 'bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-300'" class="rounded px-2 py-1 font-medium">{{ health.mode === 'enforce' ? '受控执行' : 'Shadow 观察' }}</span>
	...
	<BaseDialog :show="Boolean(labelTarget)" title="共享网络标签" width="narrow" :close-on-click-outside="true" :z-index="80" @close="closeNetworkLabel">
		<div v-if="labelTarget" class="space-y-4" data-testid="network-label-dialog">
			...
			<div><label class="input-label">网络类型</label><Select v-model="labelDraft" :options="networkLabelOptions" data-testid="network-label-select" @update:model-value="labelPreview = null" /></div>
...
const networkLabelOptions: Array<{ value: SharedNetworkLabel; label: string }> = [{ value: 'home', label: '家庭网络' }, { value: 'company', label: '公司网络' }, { value: 'school', label: '学校网络' }, { value: 'public_proxy', label: '公共代理' }, { value: 'trusted_egress', label: '可信出口' }, { value: 'mobile_cgnat', label: '移动 CGNAT' }, { value: 'unknown', label: '未知' }]
...
function networkLabelName(value?: SharedNetworkLabel) { return networkLabelOptions.find((option) => option.value === value)?.label || '未标记' }
...
async function previewNetworkLabel() { if (!labelTarget.value) return; labelSaving.value = true; labelRevokePreview.value = false; labelActionError.value = ''; try { labelPreview.value = await userRiskControlV2API.previewNetworkLabel(labelTarget.value.id, labelDraft.value) } catch (error) { labelActionError.value = error instanceof Error ? error.message : '影响预览失败' } finally { labelSaving.value = false } }
async function previewNetworkLabelRevoke() { if (!labelTarget.value?.network_label) return; labelSaving.value = true; labelActionError.value = ''; try { labelPreview.value = await userRiskControlV2API.previewNetworkLabel(labelTarget.value.id, ''); labelRevokePreview.value = true } catch (error) { labelActionError.value = error instanceof Error ? error.message : '撤销预览失败' } finally { labelSaving.value = false } }
async function applyNetworkLabel() { if (!labelTarget.value || !labelPreview.value) return; if (!labelReason.value.trim()) { labelError.value = '原因不能为空'; return } labelSaving.value = true; labelActionError.value = ''; try { await userRiskControlV2API.applyNetworkLabel(labelTarget.value.id, labelDraft.value, labelReason.value); const item = ipItems.value.find((candidate) => candidate.id === labelTarget.value?.id); if (item) { item.network_label = labelDraft.value; item.network_label_reason = labelReason.value.trim() } labelTarget.value = null; states.summary.loaded = false } catch (error) { labelActionError.value = error instanceof Error ? error.message : '标签应用失败' } finally { labelSaving.value = false } }
async function revokeNetworkLabel() { if (!labelTarget.value?.network_label) return; if (!labelReason.value.trim()) { labelError.value = '撤销原因不能为空'; return } labelSaving.value = true; labelActionError.value = ''; try { await userRiskControlV2API.revokeNetworkLabel(labelTarget.value.id, labelReason.value); const item = ipItems.value.find((candidate) => candidate.id === labelTarget.value?.id); if (item) { item.network_label = undefined; item.network_label_reason = undefined } labelTarget.value = null } catch (error) { labelActionError.value = error instanceof Error ? error.message : '标签撤销失败' } finally { labelSaving.value = false } }
