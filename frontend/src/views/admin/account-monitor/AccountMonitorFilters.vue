<template>
  <section class="space-y-3 border-y border-gray-200 py-3 dark:border-dark-700" data-testid="account-monitor-filters">
    <div class="flex flex-wrap items-center gap-3">
		<Select v-model="draft.range" class="w-full sm:w-36" :options="rangeOptions" @update:model-value="runImmediate" />
		<Select v-model="draft.platform" class="w-full sm:w-40" :options="platformOptions" @update:model-value="runImmediate" />
		<Select v-model="draft.groupID" class="w-full sm:w-48" :options="groupOptions" @update:model-value="runImmediate" />
		<SearchInput :model-value="draft.model || ''" class="w-full sm:w-52" placeholder="实际上游模型" @update:model-value="updateModel" @search="runImmediate" />
		<Select v-model="draft.accountStatus" class="w-full sm:w-36" :options="statusOptions" @update:model-value="runImmediate" />
		<Select v-model="draft.result" class="w-full sm:w-36" :options="resultOptions" @update:model-value="runImmediate" />
		<Select v-model="draft.rollup" class="w-full sm:w-36" :options="rollupOptions" @update:model-value="runImmediate" />
		<label class="flex w-full items-center gap-2 sm:w-auto"><span class="text-xs text-gray-500">风险分</span><input v-model.number="draft.minRiskScore" type="number" min="0" max="100" class="input w-20" placeholder="最低" @input="schedule" /><span class="text-gray-400">-</span><input v-model.number="draft.maxRiskScore" type="number" min="0" max="100" class="input w-20" placeholder="最高" @input="schedule" /></label>
