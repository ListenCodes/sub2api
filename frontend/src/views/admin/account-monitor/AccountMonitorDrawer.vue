<template>
  <BaseDialog :show="show" :title="account?.account_name || `账号 ${account?.account_id || ''}`" width="full" :close-on-click-outside="false" :z-index="70" @close="$emit('close')">
    <div v-if="account" class="min-w-0" data-testid="account-monitor-drawer">
      <div class="mb-4 flex flex-wrap items-center gap-3 border-b border-gray-200 pb-3 text-xs text-gray-500 dark:border-dark-700">
      ...
      <nav class="flex max-w-full gap-1 overflow-x-auto" aria-label="账号详情">
        <button v-for="item in tabs" :key="item.value" type="button" class="shrink-0 border-b-2 px-3 py-2 text-sm font-medium" :class="tab === item.value ? 'border-primary-600 text-primary-700 dark:text-primary-300' : 'border-transparent text-gray-500'" :data-testid="`account-detail-tab-${item.value}`" @click="openTab(item.value)">{{ item.label }}</button>
      </nav>
      <div class="mt-4 min-h-64 overflow-x-auto" data-testid="account-detail-content">
        <div v-if="loading" class="flex min-h-48 items-center justify-center text-sm text-gray-500" role="status">正在加载详情</div>
        <div v-else-if="error" class="flex min-h-48 flex-col items-center justify-center gap-3 text-center" role="alert"><p class="text-sm text-red-600">{{ error }}</p><button type="button" class="btn btn-secondary" data-testid="account-detail-retry" @click="load">重试</button></div>
        <template v-else-if="tab === 'media'"><div class="grid grid-cols-1 gap-3 sm:grid-cols-3"><div class="border-b border-gray-200 p-3 dark:border-dark-700"><p class="text-xs text-gray-500">图片生成</p><strong class="mt-1 block text-xl">{{ account.image_count }}</strong></div><div class="border-b border-gray-200 p-3 dark:border-dark-700"><p class="text-xs text-gray-500">视频生成</p><strong class="mt-1 block text-xl">{{ account.video_count }}</strong></div><div class="border-b border-gray-200 p-3 dark:border-dark-700"><p class="text-xs text-gray-500">视频时长</p><strong class="mt-1 block text-xl">{{ account.video_duration_seconds }} 秒</strong></div></div></template>
        ...
        <template v-else><table v-if="rows.length" class="min-w-full text-left text-sm"><thead><tr><th v-for="column in detailColumns" :key="column.key" class="whitespace-nowrap border-b border-gray-200 px-3 py-2 text-xs font-medium text-gray-500 dark:border-dark-700">{{ column.label }}</th></tr></thead><tbody><tr v-for="(row, index) in rows" :key="String(row.event_key || row.actual_model || row.user_id || row.error_category || index)"><td v-for="column in detailColumns" :key="column.key" class="whitespace-nowrap border-b border-gray-100 px-3 py-2 dark:border-dark-800">{{ cell(row, column.key) }}</td></tr></tbody></table><EmptyState v-else title="当前范围没有明细" /></template>
      </div>
    </div>
  </BaseDialog>
</template>
...
const tabs: Array<{ value: AccountDetailTab; label: string }> = [{ value: 'models', label: '模型' }, { value: 'users', label: '用户 / API Key' }, { value: 'errors', label: '错误' }, { value: 'trends', label: '趋势' }, { value: 'attempts', label: '尝试' }, { value: 'media', label: '媒体' }]
const columnsByTab: Record<string, Array<{ key: string; label: string }>> = {
  ...
  users: [{ key: 'email', label: '用户' }, { key: 'username', label: '用户名' }, { key: 'api_key_name', label: 'API Key' }, { key: 'masked_prefix', label: '脱敏前缀' }, { key: 'attempts', label: '调用' }, { key: 'successes', label: '成功' }, { key: 'tokens', label: 'Token' }],
  ...
  attempts: [{ key: 'attempted_at', label: '时间' }, { key: 'actual_model', label: '实际上游模型' }, { key: 'result', label: '结果' }, { key: 'error_category', label: '失败原因' }, { key: 'status_code', label: '状态码' }, { key: 'user_id', label: '用户 ID' }, { key: 'api_key_id', label: 'API Key ID' }, { key: 'duration_ms', label: '延迟' }],
}
    ...
    const result = props.tab === 'models' ? await accountMonitorAPI.getModels(id, props.filters)
      : props.tab === 'users' ? await accountMonitorAPI.getUsers(id, props.filters)
        : props.tab === 'errors' ? await accountMonitorAPI.getErrors(id, props.filters)
          : props.tab === 'trends' ? await accountMonitorAPI.getTrends(id, props.filters)
            : await accountMonitorAPI.getAttempts(id, props.filters)
