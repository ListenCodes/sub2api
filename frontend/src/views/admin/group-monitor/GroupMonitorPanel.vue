<template>
  <section class="min-w-0 space-y-5" data-testid="group-monitor-panel">
    <div class="flex flex-wrap items-center justify-between gap-3"><div><h2 class="text-lg font-semibold text-gray-950 dark:text-white">分组监控</h2><p class="mt-1 text-xs text-gray-500">{{ dataAsOf ? `数据截至 ${new Date(dataAsOf).toLocaleString()}` : '尚未获取聚合数据' }}</p></div><div class="flex items-center gap-2"><label class="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-300"><Toggle v-model="autoRefresh" />自动刷新</label><button type="button" class="btn btn-primary" data-testid="group-monitor-refresh" :disabled="loading" @click="load"><Icon name="refresh" size="sm" :class="{ 'animate-spin': loading }" />刷新</button></div></div>
    ...
    <div v-else-if="groups.length" class="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4" data-testid="group-monitor-grid"><GroupMonitorCard v-for="group in groups" :key="group.group_id" :group="group" @select="openGroup" /></div>
    <EmptyState v-else title="当前筛选没有分组聚合数据" />
    <Pagination v-if="total" :page="state.page" :total="total" :page-size="state.pageSize" :page-size-options="[12, 24, 48]" @update:page="setPage" @update:page-size="changePageSize" />
...
const dataAsOf = ref('')
const quality = ref<GroupMonitorQuality>({ missing_group_requests: 0, exact_model_requests: 0, estimated_model_requests: 0 })
...
onMounted(() => { void load(); timer = window.setInterval(() => { if (autoRefresh.value) void load() }, 60_000) })
onBeforeUnmount(() => { if (timer !== undefined) window.clearInterval(timer); accountMonitorAPI.dispose() })
</script>
