...
const group = { group_id: 7, name: 'OpenAI Primary', platform: 'openai', group_status: 'active', call_status: 'partial_failure' as const, total_requests: 15, successes: 12, failures: 3, success_rate: 0.8, timeline: [bucket] }
...
describe('GroupMonitorDetailDialog', () => {
  it('renders totals, alphabetical actual models, success/total cells, and bucket details', async () => {
    vi.mocked(accountMonitorAPI.getGroup).mockResolvedValue({ group, data_as_of: '2026-07-15T08:00:00Z', models: [
      { actual_model: 'z-model', total_requests: 5, successes: 4, failures: 1, exact_model_requests: 4, estimated_model_requests: 1, success_rate: 0.8, timeline: [bucket] },
      { actual_model: 'a-model', total_requests: 10, successes: 8, failures: 2, exact_model_requests: 10, estimated_model_requests: 0, success_rate: 0.8, timeline: [bucket] },
    ] })
