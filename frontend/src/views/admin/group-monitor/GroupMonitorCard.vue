<template>
  <button type="button" class="flex h-full min-h-56 w-full flex-col border border-gray-200 bg-white p-4 text-left shadow-sm transition hover:border-primary-300 hover:shadow dark:border-dark-700 dark:bg-dark-900" :data-testid="`group-card-${group.group_id}`" @click="$emit('select', group)">
    <div class="flex w-full items-start justify-between gap-3"><div class="min-w-0"><h3 class="truncate text-sm font-semibold text-gray-950 dark:text-white" :title="group.name">{{ group.name }}</h3><p class="mt-1 text-xs text-gray-500">{{ group.platform || '未知平台' }}</p></div><span class="shrink-0 rounded-md px-2 py-1 text-xs" :class="group.group_status === 'active' ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-950/30 dark:text-emerald-300' : 'bg-gray-100 text-gray-600 dark:bg-dark-700 dark:text-gray-300'">{{ group.group_status === 'active' ? '启用' : '停用' }}</span></div>
    ...
    <div class="mt-auto w-full pt-4"><GroupMonitorTimeline :buckets="group.timeline" /></div>
  </button>
</template>
...
const props = defineProps<{ group: GroupMonitorCard }>()
