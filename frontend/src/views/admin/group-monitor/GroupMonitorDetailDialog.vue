<template>
  <BaseDialog :show="show" :title="detail?.group.name || '分组详情'" width="full" :close-on-click-outside="false" :z-index="70" @close="$emit('close')">
    ...
    <div v-else-if="detail" class="min-w-0" data-testid="group-detail-dialog">
      <p v-if="originalStatus && detail.group.group_status !== originalStatus" class="mb-3 border-y border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-700 dark:border-amber-900/50 dark:bg-amber-950/20 dark:text-amber-300">状态已变化：{{ detail.group.group_status === 'active' ? '已启用' : '已停用' }}</p>
      <div class="grid grid-cols-2 border-y border-gray-200 dark:border-dark-700 sm:grid-cols-5"><div v-for="metric in metrics" :key="metric.label" class="border-b border-r border-gray-200 p-3 dark:border-dark-700 sm:border-b-0"><p class="text-xs text-gray-500">{{ metric.label }}</p><p class="mt-1 text-lg font-semibold tabular-nums">{{ metric.value }}</p></div></div>
      <p class="mt-2 text-xs text-gray-500">数据截至 {{ new Date(detail.data_as_of).toLocaleString() }}</p>
      <div class="mt-4 max-w-full overflow-x-auto" data-testid="group-model-timeline-scroll">
        <table class="min-w-max border-separate border-spacing-0 text-xs">
          <thead class="sticky top-0 z-20 bg-white dark:bg-dark-900"><tr><th class="sticky left-0 z-30 min-w-48 border-b border-r border-gray-200 bg-white px-3 py-2 text-left dark:border-dark-700 dark:bg-dark-900">实际模型</th><th v-for="bucket in headers" :key="bucket.bucket_at" class="min-w-20 border-b border-gray-200 px-2 py-2 text-center font-medium text-gray-500 dark:border-dark-700">{{ time(bucket.bucket_at) }}</th></tr></thead>
          <tbody><tr v-for="model in sortedModels" :key="model.actual_model"><th class="sticky left-0 z-10 border-b border-r border-gray-200 bg-white px-3 py-2 text-left font-medium dark:border-dark-700 dark:bg-dark-900">{{ model.actual_model }}</th><td v-for="bucket in model.timeline" :key="bucket.bucket_at" class="border-b border-gray-100 p-1 text-center dark:border-dark-800"><button type="button" class="h-9 w-full min-w-16 rounded-sm px-1 focus:outline-none focus:ring-2 focus:ring-primary-500" :class="bucketClass(bucket.status)" data-testid="model-bucket-cell" :title="bucketTitle(bucket)" @click="selectedBucket = { model: model.actual_model, bucket }">{{ bucket.successes }} / {{ bucket.total }}</button></td></tr></tbody>
        </table>
      </div>
      <div v-if="selectedBucket" class="mt-3 border-y border-gray-200 px-3 py-2 text-sm dark:border-dark-700"><strong>{{ selectedBucket.model }}</strong> · {{ new Date(selectedBucket.bucket.bucket_at).toLocaleString() }} · 总数 {{ selectedBucket.bucket.total }} · 成功 {{ selectedBucket.bucket.successes }} · 失败 {{ selectedBucket.bucket.failures }} · 成功率 {{ selectedBucket.bucket.total ? `${(selectedBucket.bucket.successes * 100 / selectedBucket.bucket.total).toFixed(1)}%` : '暂无' }}</div>
