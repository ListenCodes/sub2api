...
test('the apply phase refreshes source views before switching extensions', () => {
  ...
  const statusIndex = apply.lastIndexOf('release_job_update "$JOB_ID" switching_extensions')
  const checkoutIndex = apply.lastIndexOf('release_checkout_exact_commit')
  const sourceViewsIndex = apply.lastIndexOf('refresh_account_monitor_source_views')
  const switchIndex = apply.lastIndexOf('up -d --pull never --no-deps --force-recreate extensions-self')
  assert.ok(statusIndex >= 0 && statusIndex < checkoutIndex)
  assert.ok(checkoutIndex < sourceViewsIndex && sourceViewsIndex < switchIndex)
})
