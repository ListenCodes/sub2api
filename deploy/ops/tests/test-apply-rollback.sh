...
if [[ "${1:-}" == compose ]]; then
  ...
  if [[ "$*" == *'config --format json'* ]]; then
    ...
    jq -n --arg main "$main" --arg ext "$ext" '{name:"deploy",services:{sub2api:{image:$main,healthcheck:{},volumes:[{target:"/app/data"},{target:"/app/scripts/sync-upstream.sh"},{target:"/repo"},{target:"/usr/bin/docker"},{target:"/var/run/docker.sock"}],networks:{"sub2api-network":{}}},"extensions-self":{image:$ext,healthcheck:{},networks:{"sub2api-network":{}}},postgres:{},redis:{},"risk-control-postgres":{}},volumes:{sub2api_data:{},postgres_data:{},redis_data:{},risk_control_postgres_data:{}}}'
  elif [[ " $* " == *' up '* ]]; then
