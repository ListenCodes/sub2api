...
ledger_validate_release() {
  ...
  jq -e '
    ...
    and (if .source_kind == "identity-config" then
      (.identity_transition | IN("stage0-safe-reset", "stage1-v2", "stage1-ip", "stage1-device", "stage2-admin", "stage3-shadow-window", "stage3-rules", "stage4-geo", "stage5-composite-enforcement"))
...
_ledger_commit_release_unlocked() {
  ...
  case "$source_kind" in
	...
	identity-config)
	  ...
	  [[ "$identity_transition" =~ ^stage(0-safe-reset|1-(v2|ip|device)|2-admin|3-(shadow-window|rules)|4-geo|5-composite-enforcement)$ ]] || return 1
