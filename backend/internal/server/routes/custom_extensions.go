...
func registerCustomAdminRoutes(admin *gin.RouterGroup, h *handler.Handlers, custom *handler.CustomExtensions) {
	...
	admin.POST("/users/:id/risk-status/revoke-sessions", custom.AdminUser.RetryRiskSessionRevocation)
	...
	admin.GET("/identity-health", custom.AdminUser.ProxyRiskIdentityHealth)
	admin.POST("/risk-rebuilds/dry-run", custom.AdminUser.ProxyRiskIdentityRebuildDryRun)
	admin.POST("/risk-rebuilds", custom.AdminUser.ProxyRiskIdentityRebuild)
	admin.GET("/risk-rebuilds/:id", custom.AdminUser.ProxyRiskIdentityRebuildStatus)
